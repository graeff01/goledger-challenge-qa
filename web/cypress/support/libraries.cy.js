...
describe('Libraries', () => {
  ...
  it('[BUG-009] Erros técnicos aparecem automaticamente ao carregar a página', () => {
    ...
    // BUG: a página deveria carregar limpa, sem erros
  ...
  it('[BUG-011] Criar biblioteca não exibe nenhum feedback ao usuário', () => {
    cy.get('input[placeholder*="library" i], input[placeholder*="Library" i]').first().type('Biblioteca Teste')
    cy.get('button').contains(/create library/i).click()
    cy.wait(2000)

    // BUG: nenhuma mensagem de sucesso ou erro é exibida
    cy.contains(/success/i).should('not.exist')
    cy.contains(/criado/i).should('not.exist')
