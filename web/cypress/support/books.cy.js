...
describe('Books', () => {
  ...
  it('[BUG-001] Botão Next aparece mesmo sem resultados na busca', () => {
    ...
    // BUG: o botão Next não deveria aparecer quando não há resultados
  ...
  it('[BUG-002] Clicar em Next sem resultados dispara chamadas desnecessárias à API', () => {
    ...
    // Clica em Next e verifica se faz nova chamada mesmo sem resultados
    cy.get('button').contains(/next/i).then(($btn) => {
      if ($btn.length > 0) {
        cy.wrap($btn).click()
  ...
  it('[BUG-004] Criar livro com usuário logado retorna 401 Unauthorized', () => {
    ...
    cy.get('input[placeholder*="title" i], input[placeholder*="Go Programming" i]').type('Test Book')
    cy.get('input[placeholder*="author" i], input[placeholder*="Alan" i]').last().type('John Doe')
    cy.get('input[placeholder*="genre" i], input[placeholder*="Technology" i]').type('Technology')
    ...
    // Verifica que mensagem de erro aparece na tela
  ...
  it('[BUG-005] Mensagem de erro ao criar livro é genérica e sem contexto', () => {
    ...
    // BUG: a mensagem deveria ser específica, não genérica
